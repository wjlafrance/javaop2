...
public class PluginMain extends GenericPluginInterface implements OutgoingTextCallback, EventCallback
{
    ...
    public void activate(PublicExposedFunctions out, PluginCallbackRegister register)
    {
        ...
        out.schedule(new Callback(), 1000 * Integer.parseInt(out.getLocalSettingDefault(getName(), "Update idle", "60")));
    }
    ...
    {
        Properties p = new Properties();
        p.setProperty("Start showing idle", "This is the time (in seconds) before the first idle message is set.");
        p.setProperty("Update idle", "This is the amount of time that the idle time is updated.  If you're displaying seconds, an odd number like 17 or 29 is useful.  WARNING: don't put this so low that it backs up the flood protection! [Must reload bot to take effect]");
        p.setProperty("Show only minutes", "If this is turned off, it'll show seconds.  This is usually pretty redundant, but some people like it.");
        p.setProperty("/commands end idle", "If this is enabled, then a /command (like /whereis iago) won't reset the idle.");
        p.setProperty("Away message", "The away message.  %t is replaced with the elapsed time.  You can have more than one %t if you really want.  %t also isn't required if you just want a standard away message.");
    ...
    {
        if(settingName.equalsIgnoreCase("/commands end idle") || settingName.equalsIgnoreCase("Show only minutes"))
            return new JCheckBox("", value.equalsIgnoreCase("true"));
        else if(settingName.equalsIgnoreCase("Start showing idle") || settingName.equalsIgnoreCase("update idle"))
    ...
    private class Callback extends TimerTask
    {
        public void run()
        {
            try
            {
                ...
                if((elapsed / 1000) > Integer.parseInt(out.getLocalSettingDefault(getName(), "Start showing idle", "180")))
                {
                    String message = out.getLocalSettingDefault(getName(), "Away message", "Bot has been idle for %t");
                    
                    if(out.getLocalSettingDefault(getName(), "Show only minutes", "true").equalsIgnoreCase("true"))
                    {
                        ...
                        if(elapsed > 60)
                        {
                            ...
                            message = message.replaceAll("%t", hours + " hour" + (hours == 1 ? "" : "s") + ", " + minutes + " minute" + (minutes == 1 ? "" : "s"));
                        }
                        else
                        {
                            message = message.replaceAll("%t", elapsed + " minute" + (elapsed == 1 ? "" : "s"));
                        }
                    }
                    else
                    {
    ...
    public void sentText(String text, Object data)
    {
        try
        {
            if(out.getLocalSettingDefault(getName(), "/commands end idle", "false").equalsIgnoreCase("false") && text.matches("/.*"))
                return;
