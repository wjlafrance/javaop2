...
public class PluginMain extends GenericPluginInterface implements EventCallback, CommandCallback
{
    ...
    public void activate(PublicExposedFunctions out, PluginCallbackRegister register)
    {
        ...
        register.registerCommandPlugin(this, "loud",   2, false, "ANL", "<command/params>", "Runs a command, sending its output to the channel, regardless of what the 'loud' setting is", null);
        register.registerCommandPlugin(this, "quiet",  2, false, "ANL", "<command/params>", "Runs a command, whispering back the result, regardless of what the 'loud' setting is", null);
        register.registerCommandPlugin(this, "silent", 2, false, "ANL", "<command/params>", "Runs a command, sending output only to the console, regardless of what the 'loud' setting is", null);
        
        register.registerCommandPlugin(this, "setflagsfor", 2, false, "N", "<command> [flags]", "Sets the flags required to use a command on this bot to the specified flags.  If [flags] isn't included, it is reset to defaults.", null);
        register.registerCommandPlugin(this, "getflagsfor", 1, false, "AN", "<command>", "Retrieves the customized flags required to user a command on this bot.", null);
        
        register.registerCommandPlugin(this, "settrigger", 1, false, "N", "[new trigger]", "Sets the trigger to the specified value.  The trigger can be 0 or more characters, although a 0-character trigger gets very annoying", null);  
    }
    ...
    {
        Properties p = new Properties();
        p.setProperty("trigger", "The character or characters that, when put at the beginning of a user's message, makes it a command.  This can be blank.");
        p.setProperty("loud", "Whether or not to give responses to commands out loud.  Values are \"loud\", \"quiet\", \"loud (no name)\", or \"silent (not recommended)\", ");
        p.setProperty("stacked messages", "Whether or not to allow stacking messages with semicolons (like ?say a;say b;say c;say d)");
        p.setProperty("whispers always command", "If this is set to true, then anything whispered to the bot will be interpreted as a command.  This is handy for ops, but annoying for chatting (and can cause infinite 'command not found' loops)");
        p.setProperty("?trigger requires flags", "If this is enabled, a user must have A, N, or L flag to use ?trigger");
    ...
    public JComponent getComponent(String settingName, String value)
    {
        ...
        {
            JComboBox loud = new JComboBox(new String[] { "quiet", "loud", "loud (no name)", "silent (not recommended)" });
    ...
    public void whisperFrom(String user, String statstring, int ping, int flags) throws IOException, PluginException
    {
        ...
        {
            if(out.getLocalSettingDefault(getName(), "whispers always command", "false").equalsIgnoreCase("true"))
    ...
    {
        if(out.getLocalSettingDefault(getName(), "?trigger requires flags", "true").equalsIgnoreCase("true") && (out.dbHasAny(user, "ANL", true) == false))
            return false;
        
        if(message.equalsIgnoreCase("?trigger") == false)
            return false;
        
        out.sendTextUserPriority(user, "The trigger is " + out.getLocalSetting(getName(), "trigger"), QUIET, PRIORITY_VERY_LOW - 10);
        return true;
    }
    ...
    {
        if(out.getLocalSettingDefault(getName(), "stacked messages", "true").equalsIgnoreCase("true"))
        {
    ...
    public void commandExecuted(String user, String command, String[] args, int loudness, Object data) throws PluginException, IOException, CommandUsedIllegally, CommandUsedImproperly
    {
        ...
        else if(command.equalsIgnoreCase("quiet") || command.equalsIgnoreCase("loud") || command.equalsIgnoreCase("silent"))
        {
            if(args.length == 0)
                throw new CommandUsedImproperly("loud, quiet, and silent all need at least one parameter", user, command);
            
            int fakeLoudness = -1;
            
            if(command.equalsIgnoreCase("loud"))
            {
                fakeLoudness = LOUD;
            }
            ...
            if(fakeLoudness == -1)
            {
                out.sendTextUser(user, "Error: unknown command found in Commands plugin -- please report to iago", loudness);
                return;
            }
            
            this.runCommand(user, args[0], args.length > 1 ? args[1] : "", fakeLoudness);
        }
        else if(command.equalsIgnoreCase("setflagsfor"))
        {
            if(args.length == 0)
                throw new CommandUsedImproperly("setflagsfor requires at least one parameter", user, command);
            
            if(out.getCommandOf(args[0]).equalsIgnoreCase(args[0]) == false)
            {
                out.sendTextUser(user, "Error: that is an alias, not a command.  Try using '" + out.getCommandOf(args[0]) + "' instead.", QUIET);
                return;
            }
            ...
            out.sendTextUser(user, "Flags for '" + args[0] + "': " + oldFlags + " => " + customFlags.getNoWrite(null, args[0], "<n/a>"), loudness);
        }
        else if(command.equalsIgnoreCase("getflagsfor"))
        {
            if(args.length == 0)
                throw new CommandUsedImproperly("getflagsfor requires at least one parameter", user, command);
            ...
            out.sendTextUser(user, "Flags for '" + args[0] + "' => " + customFlags.getNoWrite(null, args[0], "<n/a>"), loudness);
        }
    }
    ...
    {
        //JComboBox loud = new JComboBox(new String[] { "quiet", "loud", "loud (no name)", "silent (not recommended)" });
        String loudness = out.getLocalSettingDefault(getName(), "loud", "quiet");
        
        if(loudness.equalsIgnoreCase("loud"))
            return LOUD;
        else if(loudness.equalsIgnoreCase("loud (no name)"))
            return LOUD_NO_NAME;
        else if(loudness.equalsIgnoreCase("silent (not recommended)"))
            return SILENT;
