...
public class PluginMain extends GenericPluginInterface implements CommandCallback
{
    ...
    public void activate(PublicExposedFunctions out, PluginCallbackRegister register)
    {
        ...
        String banfFlags = out.getLocalSettingDefault(getName(),
    ...
    public Properties getSettingsDescription()
    {
        Properties p = new Properties();
        p.setProperty(
                      "S can be banned/kicked by",
                      "The flag or choice of flags required to ban or kick somebody with the \"S\" flag.  The command to do this is .banz, which can ban anybody.  Bot must be restarted for some changes.");
        p.setProperty(
                      "F can be banned/kicked by",
                      "The flag or choice of flags required to ban or kick somebody with the \"F\" flag.  The command to do this is .ban, which can ban anybody without S.  Bot must be restarted for some changes.");
        p.setProperty(
                      "Anybody else can be banned/kicked by",
                      "The flag or flags required to ban or kick anybody else.  The command to do this is .banf, which bans anybody without F.  Bot must be restarted for some changes.");
        p.setProperty(
                      "Flags to ban outside the channel",
                      "The flag required to ban people outside the channel.  BE AWARE that this is unsafe, because it lets you override safelist, and ban, say, iago@uswest");
        return p;
    }
            ...
            Object data) throws IOException, CommandUsedImproperly, CommandUsedIllegally
    {
        ...
        if (command.toLowerCase().matches("ban.*") || command.toLowerCase().matches("kick.*"))
        {
            ...
            // stright up ban, then just ban them
            if (out.dbHasAny(user, out.getLocalSettingDefault(getName(),
                                                              "Flags to ban outside the channel",
                                                              "N"), true)
        ...
        else if (command.equalsIgnoreCase("unban"))
        {
            ...
            for (int i = 0; i < args.length; i++)
            {
                if (args[i].indexOf('*') >= 0 || args[i].indexOf('?') >= 0)
                {
                    if (!notified)
                    {
                        out.sendTextUser(user, "Sorry, can't unban by pattern yet (" + args[i]
                                + ")", loudness);
                        notified = true;
                    }
                }
    ...
    private void allowedToBan(String banner, String banned) throws CommandUsedIllegally
    {
        ...
        if (out.dbHasAny(banned, "M", true))
            throw new CommandUsedIllegally("M can't be banned", banner, "ban/kick",
                    out.dbGetFlags(banner), "N/A");

        // A user with S can be banned by somebody with N
        String sCanBeBanned = out.getLocalSettingDefault(getName(), "S can be banned/kicked by",
                                                         "N");
        if (out.dbHasAny(banned, "S", true) && out.dbHasAny(banner, sCanBeBanned, true) == false)
            throw new CommandUsedIllegally("Illegally tried to ban a Safelisted user", banner,
                    "ban/kick", out.dbGetFlags(banner), sCanBeBanned);

        // A user with F can be banned by anybody with A or N
        String fCanBeBanned = out.getLocalSettingDefault(getName(), "F can be banned/kicked by",
                                                         "ON");
        if (out.dbHasAny(banned, "F", true) && out.dbHasAny(banner, fCanBeBanned, true) == false)
            throw new CommandUsedIllegally("Illegally tried to ban a Friendlisted user", banner,
                    "ban/kick", out.dbGetFlags(banner), fCanBeBanned);

        // A user without M, S, or F can be banned by anybody with A or N
        String everybodyElse = out.getLocalSettingDefault(getName(),
                                                          "Anybody else can be banned/kicked by",
                                                          "ON");
        if (out.dbHasAny(banner, everybodyElse, true) == false)
            throw new CommandUsedIllegally("Illegally tried to ban somebody", banner, "ban/kick",
                    out.dbGetFlags(banner), everybodyElse);
    }
