...
public class SettingWizard extends JFrame implements ListSelectionListener, WindowListener
{
    ...
    public SettingWizard(PublicExposedFunctions out)
    {
        ...
        // Add the list
        GenericPluginInterface[] allPlugins = out.getStaticExposedFunctionsHandle().pluginGetAll(
                                                                                                 true);
        pluginList = new SelectPlugin[allPlugins.length];
        for (int i = 0; i < allPlugins.length; i++)
            pluginList[i] = new SelectPlugin(allPlugins[i],
                    out.pluginIsActive(allPlugins[i].getName()));
    ...
    public void valueChanged(ListSelectionEvent e)
    {
        ...
        Properties defaultSettings = out.getStaticExposedFunctionsHandle().pluginGetDefaultSettings(
                                                                                                    thisPlugin);
        Properties descriptions = out.getStaticExposedFunctionsHandle().pluginGetDescriptions(
                                                                                              thisPlugin);

        Properties settings = out.getLocalSettingSection(thisPlugin);
        // for(int i = 0; i < settingNames.length; i++)
        // settings.setProperty(settingNames[i],
        ...
        right.removeAll();
        right.add(new PluginPanel(thisPlugin, out.getStaticExposedFunctionsHandle()),
                  BorderLayout.NORTH);
        JScrollPane scroller = new JScrollPane(rightPreferences = new PreferencesPanel(settings,
