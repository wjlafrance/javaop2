...
public class QuickSettings extends JFrame implements WindowListener
{
    ...
    public QuickSettings(PublicExposedFunctions out)
    {
        ...
        right.add(new JScrollPane(rightPrefs = new PreferencesPanel(currentSettings,
                defaultSettings, descriptions, components)));
        // right.add(new JLabel("Please select a plugin to configure"),
        // BorderLayout.CENTER);
    ...
    private void saveRight()
    {
        ...
        if (JOptionPane.showConfirmDialog(this, "Save settings?", "Save?",
                                          JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
            return;

        Properties newPref = rightPrefs.getValues();

        out.putLocalSetting(" Default", "connect automatically",
                            newPref.getProperty("connect automatically"));
        ...
        out.putLocalSetting("Battle.net Login Plugin", "home channel",
                            newPref.getProperty("home channel"));
        ...
        if (out.pluginIsActive("Commands"))
        {
            ...
            out.putLocalSetting("Commands", "stacked messages",
                                newPref.getProperty("stacked messages"));
            out.putLocalSetting("Commands", "trigger", newPref.getProperty("trigger"));
            out.putLocalSetting("Commands", "whispers always command",
                                newPref.getProperty("whispers always command"));
        }
        ...
        // out.putLocalSetting(currentPlugin, keys[i],
        // newPref.getProperty(keys[i]));
    }
}
