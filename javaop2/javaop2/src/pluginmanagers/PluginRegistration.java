...
public class PluginRegistration implements PluginCallbackRegister
{
    ...
    public boolean raiseCommand(String user, String command, String args, int loudness,
            boolean errorOnUnknown) throws PluginException, CommandUsedImproperly, IOException, CommandUsedIllegally
    {
        String newCommand;

        int infiniteLoopChecker = 0;
        while ((++infiniteLoopChecker < 1000)
                && (newCommand = getCommandOf(command)).replaceAll(" .*", "").equalsIgnoreCase(
                                                                                               command) == false)
        {
        ...
        if (out.dbHasAny(user, requiredFlags, true) == false)
            throw new CommandUsedIllegally("User attempted to use an illegal command", user,
                    command, out.dbGetFlags(user), requiredFlags);
    ...
    public BNetEvent eventOccurring(BNetEvent event) throws IOException, PluginException
    {
        ...
        while (e.hasMoreElements())
        {
            ...
            if ((event = ((RawEventCallback) plugin.getCallback()).eventOccurring(event,
                                                                                  plugin.getData())) == null)
                return null;
        }
            ...
            KeyStroke hotkey, Icon icon, ActionListener callback)
    {
        ...
        while (e.hasMoreElements())
        {
            ...
            ((GuiCallback) plugin.getCallback()).menuItemAdded(name, whichMenu, index, mnemonic,
                                                               hotkey, icon, callback,
                                                               plugin.getData());
        }
        // System.exit(0);
    }
    ...
    public void userMenuAdded(String name, int index, Icon icon, ActionListener callback)
    {
        ...
        {
            GuiPlugin plugin = (GuiPlugin) e.nextElement();
            ((GuiCallback) plugin.getCallback()).userMenuAdded(name, index, icon, callback,
                                                               plugin.getData());
        }
    }
    ...
    public void userAdded(String username, String flags)
    {
        ...
        while (e.hasMoreElements())
        {
            ...
            ((UserDatabaseCallback) plugin.getCallback()).userAdded(username, flags,
                                                                    plugin.getData());
        }
    }
    ...
    public void userChanged(String username, String oldFlags, String newFlags)
    {
        ...
        {
            UserDatabasePlugin plugin = (UserDatabasePlugin) e.nextElement();
            ((UserDatabaseCallback) plugin.getCallback()).userChanged(username, oldFlags, newFlags,
                                                                      plugin.getData());
        }
    }
    ...
    public void userRemoved(String username, String oldFlags)
    {
        ...
        {
            UserDatabasePlugin plugin = (UserDatabasePlugin) e.nextElement();
            ((UserDatabaseCallback) plugin.getCallback()).userAdded(username, oldFlags,
                                                                    plugin.getData());
        }
    }
            ...
            String command)
    {
        ...
        while (e.hasMoreElements())
        {
            ...
            ((UserErrorCallback) plugin.getCallback()).illegalCommandUsed(user, userFlags,
    ...
    public void unknownCommandUsed(String user, String command)
    {
        ...
        {
            UserErrorPlugin plugin = (UserErrorPlugin) e.nextElement();
            ((UserErrorCallback) plugin.getCallback()).nonExistantCommandUsed(user, command,
                                                                              plugin.getData());
        }
    }
            ...
            String errorMessage)
    {
        ...
        {
            UserErrorPlugin plugin = (UserErrorPlugin) e.nextElement();
            ((UserErrorCallback) plugin.getCallback()).commandUsedImproperly(user, command,
                                                                             syntaxUsed,
                                                                             errorMessage,
                                                                             plugin.getData());
        }
