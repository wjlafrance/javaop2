...
public class PluginMain extends GenericPluginInterface implements CommandCallback, PacketCallback, EventCallback
{
    ...
    public void activate(PublicExposedFunctions out, PluginCallbackRegister register)
    {
        ...
        register.registerCommandPlugin(this, "sweepban", 1, true, "ON", "<channel>", "Bans everybody in the selected channel.  Two channels may not be sweepbanned at the same time.", null);
        register.registerCommandPlugin(this, "sweepbanip", 1, true, "ON", "<channel>", "Bans everybody in the selected channel based on their ip.  Two channels may not be sweepbanned at the same time.", null);
    ...
    {
        if(args.length == 0)
            throw new CommandUsedImproperly("Sweepban requires a parameter (the channel name)", user, command);
        ...
        // Send a profile request so we can tell when it's done (thanks to Stealth)
