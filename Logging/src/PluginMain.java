 ...
 *
 */
public class PluginMain extends GenericPluginInterface implements EventCallback, OutgoingTextCallback, SystemMessageCallback, ErrorCallback, PacketCallback
{
    ...
    public Properties getSettingsDescription()
    {
        ...
        p.setProperty("Log packets", "Logs incoming and outgoing packets.  Use this to submit a connection problem, but be warned: it'll generate HUGE logs.  Bot must be restarted for this to take effect (simply because of the overhead, it slows things down a little)");
        p.setProperty("Log join/leave", "Logs notifications when users join or leave the channel");
        p.setProperty("Log format", "The format of the logs.  Either \"text\", \"html\" or \"bb\" (bb is for posting on forums)");
        p.setProperty("Log file", "The full path to where you want the logs to go.  %d will be expanded to the date "
    ...
    {
        if (settingName.equalsIgnoreCase("Log join/leave") || settingName.equalsIgnoreCase("Log packets"))
        {
            return new JCheckBox("", value.equalsIgnoreCase("true") ? true : false);
        }
        else if (settingName.equalsIgnoreCase("Log format"))
        {
            JComboBox list = new JComboBox(new String[] { "html", "text", "bb" });
    ...
    /** This function will expect to receive strings in html format, <color="blue"> [text] </color> format. */
    private void addToLog(String text)
    {
        try
        {
            String format = out.getLocalSettingDefault(getName(), "Log format", "html");
            String filename = out.getLocalSettingDefault(getName(), "Log file", new RelativeFile("logs/%n-%d.html").getAbsolutePath());
            ...
            if (format.equalsIgnoreCase("html"))
                output.println("<b><font color=\"white\">" + Timestamp.getTimestamp() + "</font></b> " + text + "<br>");
            else if (format.equalsIgnoreCase("bb"))
                output.println("[b][color=white]" + Timestamp.getTimestamp() + "[/color][/b]" + text);
            else
    ...
    {
        addToLog(format("<" + out.getLocalVariable("username") + "> ", "cyan") + format(text, "white"));
    }
