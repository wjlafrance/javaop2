...
/*
 * Created on Jan 15, 2005 By iago
 */
		...
		UserDatabaseCallback
{
	...
	public String getLongDescription() {
		return "Bans users who enter the channel who B but not S, or that have "
				+ "Z.  It also autobans users when their flags are changed to "
				+ "include B or Z.";
	}
	...
	private void tryAutoban(String user) throws IOException {
		...
		if (out.dbHasAny(user, "Z", false)) {
			out.sendTextPriority("/ban " + user + " Autoban (Z)",
					PRIORITY_VERY_HIGH);
			return;
		}
		...
		if (out.dbHasAny(user, "B", false)) {
			if (out.dbHasAny(user, "S", false) == false) {
				out.sendTextPriority("/ban " + user + " Autoban (B)",
						PRIORITY_HIGH);
				return;
			}
		}
