...
public class VerHash
{
			...
			throws IOException
	{
		...
		/*
		 * A pointer to the table of sections. This, along with the number of sections, is passed to the next function.
		 */
		int ptrSectionTable;
	 ...
	 */
	private static int processSections(MappedByteBuffer file, int sectionsBase, int numberOfSections)
			throws IOException
	{
	...
	{
		/*
		 * The address of the next node, or the address of the data. The left-most bit tells us which address this
		 * actually is.
		 */
		...
		else
		{
			...
			if(tree.get(0) == Integer.valueOf(RT_VERSION))
			{
				...
				return (buffer[0x3C] << 0) & 0x000000FF | (buffer[0x3E] << 8) & 0x0000FF00 | (buffer[0x38] << 16)
						& 0x00FF0000 | (buffer[0x3A] << 24) & 0xFF000000;
			}
		}
