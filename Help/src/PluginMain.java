 ...
 *
 */
public class PluginMain extends GenericPluginInterface implements CommandCallback, UserErrorCallback
{
    ...
    public void activate(PublicExposedFunctions out, PluginCallbackRegister register)
    {
        ...
        register.registerCommandPlugin(this, "helpall", 0, false, "ANL", "[command(s)]", "Gives help on the specified command(s). If no commands are specified, it lists all commands.", null);
        register.registerCommandPlugin(this, "help", 0, false, "ANL", "[command(s)]", "Gives help on the specified command(s). If no commands are specified, it lists all commands that you can use.", null);
        register.registerCommandPlugin(this, "usage", 0, false, "ANL", "[command(s)]", "Gives the usage for the specified command(s).", null);
    ...
    {
        Properties p = new Properties();
        p.setProperty("Command not found", "Whispers a user if they use a command that doesn't exist.  Very annoying.");
        p.setProperty("Proper usage", "Whispers a user if they use a command incorrectly.  Very handy.");
        return p;
    }
    ...
    {
        if(command.equalsIgnoreCase("helpall") || command.equalsIgnoreCase("help") || command.equalsIgnoreCase("usage"))
        {
            if(args.length == 0)
            {
        ...
        {
            out.sendTextUser(user, "There is an error in the \"Help\" plugin.  Please report this to iago.", loudness);
        }
    }
    ...
    public void sendHelp(String user, String command, int loudness) throws IOException
    {
        if(out.getCommandHelp(command) == null)
            ...
            out.sendTextUserPriority(user, command + " " + out.getCommandUsage(command) + " - " + out.getRequiredFlags(command) + " - " + out.getCommandHelp(command), loudness, PRIORITY_LOW);
    }
    ...
    public void sendUsage(String user, String command, int loudness) throws IOException
    {
        if(out.getCommandUsage(command) == null)
            ...
            out.sendTextUser(user, "Usage: " + command + " " + out.getCommandUsage(command), loudness);
    }

    public void illegalCommandUsed(String user, String userFlags, String requiredFlags, String command, Object data)
    {
    }
    ...
    public void nonExistantCommandUsed(String user, String command, Object data)
    {
        try
        {
            if(out.getLocalSettingDefault(getName(), "Command not found", "false").equalsIgnoreCase("true"))
                out.sendTextUserPriority(user, "Command \"" + command + "\" not found.", QUIET, PRIORITY_VERY_LOW);
        }
        catch(Exception e)
        {
            // *shrug*
    ...
    public void commandUsedImproperly(String user, String command, String syntaxUsed, String errorMessage, Object data)
    {
        try
        {
            if(out.getCommandUsage(command) == null)
                ...
                out.sendTextUser(user, errorMessage + " [Proper usage: " + command + " " + out.getCommandUsage(command) + "]", QUIET);
        }
        catch(Exception e)
        {
            // *shrug*
