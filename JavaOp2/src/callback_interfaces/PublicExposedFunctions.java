...
/** This is a public interface to the bot.  This interface will be passed around a lot, and all the plugins will have direct
 * access to it.
 * @author iago
 *
 */
public interface PublicExposedFunctions
{
    ...
    /** Sends a single packet or BNetPacket to battle.net.  No flood protection happens on this. */
    public void sendPacket(Buffer packet) throws IOException;
    /** Sends a single "chat" string to the channel.  This will be sent raw, assuming no filtering plugins change it. */
    public void sendText(String text) throws IOException;
    /** Sends a single "chat" string to the channel with a priority.  This is better to use. */
    public void sendTextPriority(String text, int priority) throws IOException;
    /** Sends a "chat" string directed at a user.  Generally as a whisper.  If this message is too long, it will be split
     * up into smaller messages which are sent individually. */
    public void sendTextUser(String user, String text, int loudness) throws IOException;
    /** Sends a "chat" string at a user, splitting if necessary, with a specific priority. */
    public void sendTextUserPriority(String user, String text, int loudness, int priority) throws IOException;
    
    /** Turn off the Nagle Algorithm on the socket.  This is recommended for pings. */
    public void setTCPNoDelay(boolean noDelay) throws IOException;
    
    /** Sends a system message to any handlers, the levels are defined in ErrorLevelConstants. */
    public void systemMessage(int level, String message);
    /** Sends a message to any System Message handlers.  May include codes from util.ColorCodes. */
    ...
    /** Gets a handle to StaticExposedFunctions.  This is merely here for convenience */
    ...
    /** Adds a user to the channel with the specified stats.  Returns a "User" object for them. */
    public User channelAddUser(String name, int flags, int ping, String message);
    /** Removes a user from the channel, based on their name, and returns a user object representing them.
    ...
    /** Gets a string list of all users in the channel that have any of the specified flags. */
    public String []channelGetListWithAny(String flags);
    /** Gets a string list of all users in the channel that have all of the specified flags. */
    public String []channelGetListWithAll(String flags);
    /** Gets a string list of all users in the channel that have none of the specified flags. */
