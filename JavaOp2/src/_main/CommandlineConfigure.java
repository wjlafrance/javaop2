...
public class CommandlineConfigure
{
    ...
    private static void configureLoadedBots() throws IOException
    {
        while (true)
        {
            ...
            {
                String []bots = getAndPrintAllBots(true);
                int bot = getNumericInput("Which would you like to load at startup?", null, bots.length);
    ...
    private static void manageBot(String bot) throws IOException
    {
        while (true)
        {
            ...
            else if (choice == 2)
            {
                ...
                int plugin = getNumericInput("Please select a plugin to remove", null, plugins.length);
                if (plugin != 0)
            ...
            else if (choice == 5)
            {
                ...
                int plugin = getNumericInput("Please select a plugin to configure", null, plugins.length);
                ...
                if (plugin != 0)
                {
                    ...
                    for (int i = 0; i < keys.length; i++)
                    {
                        ...
                        settings.set(thisPlugin.getName(), (String) keys[i], getPatternInput("New value?", currentSetting, ".*"));
                    }
                }
            ...
            else if(choice == 6)
            {
                ...
                int num = getNumericInput("Please select a plugin to get info on", null, plugins.length);
                if (num != 0)
                {
                    num--;
    ...
    private static int getNumericInput(String prompt, String defaultIn, int max)
    {
        while (true)
        {
            int input = Integer.parseInt(getPatternInput(prompt + " (an integer 0 and " + max + ")", defaultIn, "[0-9]+"));
    ...
    private static Object[] sortEnumeration(Enumeration e)
    {
