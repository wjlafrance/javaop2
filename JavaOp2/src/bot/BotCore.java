...
/** A single instance of this is created for each bot we're running.  It implements PublicExposedFunctions, which the only important
 * thing that is passed to our workers (plugins).
 * This class looks after:
 * - Storing the UserList
 * - Storing the settings
 * - Storing the variables
 * - Stores the socket, along with OutputStream for it
 * - Creates and stores the PacketThread, handing the InputStream for the socket to the PacketThread
...
public class BotCore implements PublicExposedFunctions
{
    ...
    final protected Hashtable localVariables;
    ...
    final protected Hashtable timerTasks = new Hashtable();
    ...
    /** If an IOException is thrown from here, it means that the localSettings file has been generated.
     * The program should end and/or prompt the user to fill out his information. */
    ...
    public void sendTextUserPriority(String user, String text, int loudness, int priority) throws IOException
    {
        ...
        while(e.hasMoreElements())
        {
            if(loudness == LoudnessConstants.LOUD)
                ...
                systemMessage(ErrorLevelConstants.ERROR, "Unknown 'loudness' setting for message: " + e.nextElement() + " (loudness was " + loudness + ")");
        }
    ...
    public void schedule(TimerTask task, long interval)
    {
        ...
        systemMessage(ErrorLevelConstants.DEBUG, "Scheduled timer: " + task + " (every " + interval + "ms)");
    }
    ...
    /** Get the command associated with an alias.  Returns the command itself if there is no alias. */
    ...
    /** Kill the bot instance with the specified name.  IllegalArgumentException is thrown if there is no 
     * bot with that name, or if the bot isn't running. */
    ...
    /** Start a new bot instance with the specified name.  IllegalArgumentException is thrown if there is no 
     * bot with that name, or if the bot is already running. */
    ...
    public String getLocalSetting(String section, String key)
    {
        ...
        {
            systemMessage(ErrorLevelConstants.ERROR, "Attempted to access a missing setting: " + section + " // " + key);
            return null;
        }
    ...
    /** Disconnects from the server, and null's out the input/output stream.
     * Note that there'll likely be a plugin to automatically reconnect you when this happens. */
    ...
    public boolean raiseCommand(String user, String command, String args, int loudness, boolean errorOnUnknown) throws IOException
    {
        ...
        {
            exceptionIllegalCommandUsed(user, exception.getUserFlags(), exception.getRequiredFlags(), exception.getCommand());
        }
        catch(CommandUsedImproperly exception)
        {
            exceptionCommandUsedImproperly(exception.getUser(), exception.getCommand(), command + " " + args, exception.getMessage());
        }
