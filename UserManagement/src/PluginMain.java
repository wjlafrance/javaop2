...
public class PluginMain extends GenericPluginInterface implements CommandCallback
{
    ...
    {
        this.out = out;
        this.mymap = new PersistantMap(new RelativeFile(out.getName() + ".level"), "This is the file that defines text flags.");

        register.registerCommandPlugin(this, "whoami", 0, false, "AGNL", "", "Shows you all the flags you have", null);
        register.registerCommandPlugin(this, "setuser", 0, false, "AGN", "<userlist> <flags>", "Adds or removes flags from a user.  The flag list is a set of +/- and flags, like +ABC-D+E-F", null);
        register.registerCommandPlugin(this, "deluser", 0, false, "AGN", "<userlist>", "Deletes a user from the database", null);
        register.registerCommandPlugin(this, "find", 0, false, "AGN", "<userlist>", "Finds the flags for the requested user", null);
        register.registerCommandPlugin(this, "findattr", 1, false, "AGN", "<flag>", "Lists all users with the specified flag", null);
        register.registerCommandPlugin(this, "whatcanichange", 0, false, "AGNL", "", "Tells you which flags you're allowed to modify", null);
        register.registerCommandPlugin(this, "setaccesslevel", 2, false, "A", "[access level, flags]", "Adds a new access level with flags.", null);
        register.registerCommandPlugin(this, "useaccesslevel", 2, false, "A", "[user, access level]", "Sets a user to a access level.", null);
        register.registerCommandPlugin(this, "deleteaccesslevel", 1, false, "A", "[access level]", "Deletes an access level. All users in this access level will remain.", null);
        register.registerCommandPlugin(this, "listaccesslevels", 0, false, "A", "", "Gives a list of all access levels.", null);
    }
    ...
    {
        Properties p = new Properties();
        p.setProperty("A can change", "This is the lowest flag for members, they can typically change only low level flags.");
        p.setProperty("G can change", "I put this in as a mid-range access modifier.  Typically, it'll be more powerful than A, but less than N.");
        p.setProperty("N can change", "This is typically used for a local master (ie, master of this bot).  Can normally change almost everything.");
        p.setProperty("P can be modified by", "Somebody with the \"P\" flag set can't generally be changed unless you're a master.");
    ...
    public void commandExecuted(String user, String command, String[] args, int loudness, Object data) throws PluginException, IOException, CommandUsedIllegally, CommandUsedImproperly
    {
        ...
        {
            if (args.length < 1)
                throw new CommandUsedImproperly("Find requires at least one parameter.", user, command);

            for (int i = 0; i < args.length; i++)
                out.sendTextUserPriority(user, args[i] + ": " + out.dbGetFlags(args[i]), loudness, PRIORITY_LOW);
        }
        else if (command.equalsIgnoreCase("deluser"))
        {
            if (args.length < 1)
                throw new CommandUsedImproperly("Deluser requires at least one parameter", user, command);
            ...
            for (int i = 0; i < args.length; i++)
            {
                ...
                if (out.dbUserExists(thisUser) == false)
                    out.sendTextUser(user, "Sorry, user \"" + thisUser + "\" not found in the database.", loudness);
                else
                    changeAndNotify(user, thisUser, funcs.getGlobalSettingDefault(getName(), "all flags", "ABCDEFGHIJKLMNOPQRSTUVWXYZ"), "", loudness);
            }
        }
        else if (command.equalsIgnoreCase("setuser"))
        {
            if (args.length < 2)
                throw new CommandUsedImproperly("Setuser requires at least two parameters", user, command);
        ...
        else if (command.equalsIgnoreCase("whatcanichange"))
        {
            ...
            out.sendTextUser(user, "You're allowed to change flags " + changes + ", and can " + (out.dbHasAny(user, out.getLocalSetting(getName(), "P can be modified by"), true) ? "" : "not ") + "modify users with P flag set.", loudness);
        }
        else if (command.equalsIgnoreCase("findattr"))
        {
            if (args.length != 1 || args[0].length() != 1)
                throw new CommandUsedImproperly("findattr requires a single character parameter", user, command);
        ...
        {
            if (args.length != 2)
                throw new CommandUsedImproperly("setaccesslevel requires two parameters.", user, command);
        ...
        else if (command.equalsIgnoreCase("useaccesslevel"))
        {
            ...
            if (args.length != 2)
                throw new CommandUsedImproperly("useaccesslevel requires two parameters.", user, command);
            ...
            {
                changeAndNotify(user, args[0], funcs.getGlobalSettingDefault(getName(), "all flags", "ABCDEFGHIJKLMNOPQRSTUVWXYZ"), flags, loudness);
            }
        }
        else if (command.equalsIgnoreCase("deleteaccesslevel"))
        {
            if (args.length != 1)
                throw new CommandUsedImproperly("deleteaccesslevel requires one parameter.", user, command);
        ...
        {
            out.sendTextUser(user, "Error: There was an unknown command.  Please report a problem with UserManagement plugin.", loudness);
        }
    ...
    private String getAllowedChanges(String user) throws PluginException
    {
        ...
        if (out.dbHasAny(user, "", true))
            flags += out.getLocalSettingDefault(getName(), "all flags", "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        ...
        if (out.dbHasAny(user, "N", true))
            flags += out.getLocalSettingDefault(getName(), "N can change", "ABCDEFGHIJKLNOPQRSTVWXYZ");
     ...
     */
    private void changeAndNotify(String changer, String userToChange, String flagsToRemove, String flagsToAdd, int loudness) throws IOException, PluginException
    {
        ...
        if (out.dbHasAny(userToChange, "P", true))
        {
            ...
            if (out.dbHasAny(changer, pOverride, true) == false)
            {
                ...
                out.sendTextUser(changer, "Sorry, users with the P flag can only be changed by somebody with " + pOverride + ".", loudness);
                return;
            }
        }
        ...
        {
            out.sendTextUser(changer, "Sorry, couldn't find any flags that you're allowed to add/remove.", loudness);
            return;
        }
        ...
        out.sendTextUser(changer, userToChange + ": " + oldFlags + " => " + newFlags + (oldFlags.equals(newFlags) ? " (no change)" : ""), loudness);
    }
