...
public class War3Statstring
{
    private static String getRank(String rank, int i) throws java.lang.ArrayIndexOutOfBoundsException
    {
        ...
        if (rank.equals("R"))
        {
            String[] ranks = { "Unknown", "Peon", "Green Dragon Whelp", "Blue Dragon", "Red Dragon", "Deathwing" };
            rankResult = ranks[i];
        }
        else if (rank.equals("O"))
        {
            String[] ranks = { "Unknown", "Peon", "Grunt", "Tauren", "Far Seer", "Thrall" };
            rankResult = ranks[i];
        }
        else if (rank.equals("U"))
        {
            String[] ranks = { "Unknown", "Peon", "Ghoul", "Abomination", "Lich", "Tichondrius" };
            rankResult = ranks[i];
        }
        else if (rank.equals("H"))
        {
            String[] ranks = { "Unknown", "Peon", "Footman", "Knight", "Archmage", "Medivh" };
            rankResult = ranks[i];
        }
        else if (rank.equals("N"))
        {
            String[] ranks = { "Unknown", "Peon", "Archer", "Druid of the Claw", "Priestess of the Moon", "Furion Stormrage" };
            rankResult = ranks[i];
        }
        else
        {
            String[] ranks = { "Unknown", "Peon", "Unknown", "Unknown" };
            rankResult = ranks[i];
        }
        return rankResult;
    }
    ...
    public static String getWar3(String message)
    {
        ...
        System.out.println("DEBUGGING LOBO:" + war3Stats[0] + " " + war3Stats[1] + " " + war3Stats[2]);
        ...
        if (war3Stats.length == 2)
        {
            ...
            return "Stats: Warcraft III(level " + war3Stats[2] + ", Best Race " + race + ", Icon" + icon + ", Clan " + clan.toString() + ")";
        }
        else if (war3Stats.length == 1)
        {
            return "Stats: Warcraft III(level " + war3Stats[2] + ", Best Race " + race + ", Icon" + icon + ")";
        }
