...
/** Every plugin has to have a class called "PluginMain" in the default folder that extends this class.  If it doesn't, then
...
abstract public class GenericPluginInterface
{
    ...
    /** This should return a Properties with each setting, as well as what it means. 
     * This is used for local settings. */
    abstract public Properties getSettingsDescription();
    /** This should return the appropriate component for the setting, along with the default value set.
     * If a JTextField is needed, null can be returned. */
    ...
    /** This is just to simplify some code */
    public Hashtable getGlobalComponents(Properties values)
    {
        ...
        for(int i = 0; i < keys.length; i++)
        {
            JComponent component = getGlobalComponent(keys[i], values.getProperty(keys[i].toLowerCase())); 
    ...
    /** The PreferencePanels class can be overridden to change how your preferences look */
    public JPanel getPreferenceImplementation(Properties currentSettings, Properties defaultSettings, Properties descriptions, Hashtable components)
    {
        return new PreferencesPanel(currentSettings, defaultSettings, descriptions, components);
    }
    
    /** The PreferencesPanel class for the global settings can also be overridden, if desired */
    public JPanel getGlobalPreferenceImplementation(Properties currentSettings, Properties defaultSettings, Properties descriptions, Hashtable components)
    {
        return new PreferencesPanel(currentSettings, defaultSettings, descriptions, components);
    }
    
    
    /** Get the full name that uniquely identified this plugin (generally, the name + version) */
    ...
    public void setDefaultSettings(PublicExposedFunctions out)
    {
        ...
        {
            out.getLocalSettingDefault(getName(), defaultSettings[i], props.getProperty(defaultSettings[i]));
        }
    ...
    public void setGlobalDefaultSettings(StaticExposedFunctions funcs)
    {
        ...
        String []defaultSettings = Uniq.uniq(props.keys());
        for(int i = 0; i < defaultSettings.length; i++)
            funcs.getGlobalSettingDefault(getName(), defaultSettings[i], props.getProperty(defaultSettings[i]));
    }
