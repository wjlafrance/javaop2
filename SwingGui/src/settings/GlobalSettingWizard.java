...
public class GlobalSettingWizard extends JFrame implements ListSelectionListener, WindowListener
{
    ...
    public GlobalSettingWizard(StaticExposedFunctions funcs)
    {
        ...
        this.getContentPane().add(new JScrollPane(list = new JList(usefulPlugins)), BorderLayout.WEST);
    ...
    public void valueChanged(ListSelectionEvent e)
    {
        ...
        for(int i = 0; i < settingNames.length; i++)
            settings.setProperty(settingNames[i], funcs.getGlobalSettingDefault(thisPlugin, settingNames[i], defaultSettings.getProperty(settingNames[i])));
        ...
        form.add(oldRight = new JScrollPane(rightPreferences = new PreferencesPanel(settings, defaultSettings, descriptions, components)), BorderLayout.CENTER);
        ...
        oldRight.getVerticalScrollBar().getModel().setValue(oldRight.getVerticalScrollBar().getModel().getMinimum());
    }
